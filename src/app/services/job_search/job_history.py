...
class LocalJobHistory:
    def __init__(
        self, history_file_path: str, history_filename: str = "job_history.json"
    ):
        self.history_file_path = history_file_path
        self.history_file_name = history_filename
        self.filename = os.path.join(history_file_path, history_filename)

    def filter_and_commit_new_jobs(
        self, incoming_jobs: List[JobPosting]
    ) -> List[JobPosting]:
        ...
        for job in incoming_jobs:
            ...
            if pair not in seen_pairs:
                ...
                new_records_to_append.append(
                    {"job_id": job_id_str, "provider": job.provider_name}
                )
        ...
        if new_records_to_append:
            ...
            write_json_list_to_file(
                all_data, self.history_file_path, self.history_file_name
            )
