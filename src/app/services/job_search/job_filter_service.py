...
class JobFilterService:
    ...
    def resolve_resume_metadata(self) -> ResumeMetadata:
        ...
        if extract_flag == "N":
            ...
            print(
                f"[Warning] Cache file requested but not found at {cache_path}. Falling back to source extraction."
            )

        # BRANCH Y: Parse from scratch and save to the cache directory
        if not source_path or not os.path.exists(source_path):
            raise FileNotFoundError(
                f"Source resume PDF file not found at: {source_path}"
            )
    ...
    def filter_jobs(self, raw_job_pool: List[JobPosting]) -> List[JobPosting]:
        ...
        banned_industries = self._parse_list_from_config(
            "EXCLUSIONS", "banned_industries"
        )
        banned_types = self._parse_list_from_config(
            "EXCLUSIONS", "banned_company_types"
        )
        ...
        for job in raw_job_pool:
            ...
            if job.industry.lower().strip() in banned_industries:
                print(
                    f" -> Skipped [Job ID {job.job_id}]: Industry '{job.industry}' is explicitly banned."
                )
                continue
            ...
            if is_type_banned:
                print(
                    f" -> Skipped [Job ID {job.job_id}]: Company type '{job.company_type}' matches banned criteria."
                )
                continue
            ...
            if job.required_years_experience > resume_md.total_years_experience:
                print(
                    f" -> Skipped [Job ID {job.job_id}]: Experience mismatch. Required: {job.required_years_experience}y, Candidate has: {resume_md.total_years_experience}y."
                )
                continue
        ...
        print(
            f"Pipeline Completed: {len(filtered_pool)} / {len(raw_job_pool)} items passed the criteria gates.\n"
        )
