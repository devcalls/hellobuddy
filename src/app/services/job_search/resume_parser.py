...
class ResumeParserEngine:
    ...
    def read_resume_metadata_from_cache(self, cache_path_dir: str) -> ResumeMetadata:
        ...
        if not os.path.exists(cache_path):
            raise FileNotFoundError(
                f"Resume metadata cache file not found at: {cache_path}"
            )
    ...
    def save_resume_metadata(
        ...
        output_dir: str,
        output_filename: str = RESUME_METADATA_FILE_NAME,
    ):
        """
        ...
        print(
            f"[Cache System] Successfully stored anonymized profile cache at: {full_output_path}"
        )
...
if __name__ == "__main__":
    ...
    SAMPLE_PDF_PATH = ""
    # ConfigManager().get_value("STORAGE", "RESUME_FILE_PATH")  # Validates presence of the key
