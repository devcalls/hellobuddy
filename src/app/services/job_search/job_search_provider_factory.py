...
class JobSearchProviderFactory:
    ...
    def get_active_providers(self):
        active_providers = []
        if (
            self.settings.api_keys.adzuna_app_id is not None
            and self.settings.api_keys.adzuna_app_key is not None
        ):
            active_providers.append(
                AdzunaProvider(
                    self.settings.api_keys.adzuna_app_id,
                    self.settings.api_keys.adzuna_app_key,
                )
            )
